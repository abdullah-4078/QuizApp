    ...
    firebase.database().ref('Questions').once("value", function (data) {
        ...
        for (var property in data) {
            if (data.hasOwnProperty(property)) {
                ...
                questions[qNum].question = data[property].Question
                questions[qNum].answer = data[property].Answer
                questions[qNum].options = data[property].Options

                var key = data[property].key
                var numOfOptns = questions[qNum].options.length
                var optionVals = []
                var questionVal = questions[qNum].question
                for (var i = 0; i < numOfOptns; i++)
                    optionVals[i] = questions[qNum].options[i]
                var answerVal = questions[qNum].answer
                ...
                " + (qNum + 1) + "><h3 class='quizHeader'>Q" + (qNum + 1) + ":&nbsp" + questionVal + "</h3>"
...
function appendResult() {
    ...
    for (var i = 0; i < questions.length; i++) {
        if (answers[i]) {
            divBody += "<td><img style='width:20px' src='Image/check.png'></td>"
            correctQ++
        }
        else
            divBody += "<td><img style='width:20px' src='Image/cancel.png'></td>"
    }
    divBody += "</tbody></table>"
    ...
    if (!(questionVal === '') && !(answerVal === '')) {
        ...
        if (enteredOpt != 2)
            ...
            if (!enteredOpt)
                ...
                firebase.database().ref('Questions/' + key).set(data)
            }
        }
    }
    else
...
function EditRec(id) {
    if (!addingRec) {
        ...
        optionsUl.innerHTML += ansLiHtml
        var btnLiHtml = "<li style='display: flex; justify-content: center;'>"
        btnLiHtml += "<button class='btn btn-success liBtnTAdd fa fa-check' id='" + key + "' onclick='AddRec(this)'></button>"
        btnLiHtml += "<button class='btn btn-danger liBtnAdd fa fa-times' id='" + key + "' onclick='cancelUpdateRec(this)'></button>"
        btnLiHtml += "</li>"
        optionsUl.innerHTML += btnLiHtml
        li.appendChild(optionsUl)
